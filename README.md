...
- **修复农历显示**：容器缺少 `zhdate` 时不再静默显示"农历未知"，计算失败会记录日志便于排查
- **修复漫画封面渲染**：新番封面渲染前预下载并转为 base64 data URI，避免 Playwright 截图时因外链超时/防盗链/CDN 波动导致封面缺失
- **新增 .gitignore**：排除运行数据、缓存、数据库、日志和本地环境文件，避免敏感配置误提交
